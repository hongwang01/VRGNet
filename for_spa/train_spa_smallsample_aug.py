...
parser.add_argument('--log_dir', default='./aug05_spalogs/', help='tensorboard logs')   # 05 means: fake_ratio=0.5
parser.add_argument('--model_dir',default='./aug05_spamodels/',help='saving model')     # 05 means: fake_ratio=0.5
...
def train_model(net, netED, datasets, optimizer, lr_scheduler):
    ...
    for epoch in range(opt.resume,opt.niter):
        ...
        lr_scheduler.step()
        # save model
        save_path_model = os.path.join(opt.model_dir, 'Aug05_DerainNet_state_'+str(epoch+1)+'.pt')  # 05 means: fake_ratio=0.5
...
if __name__ == "__main__":
    ...
    if opt.resume:  # from opt.resume continue to train opt.resume=0 from scratch
        netDerain.load_state_dict(torch.load(os.path.join(opt.model_dir, 'Aug05_DerainNet_state_' + str(opt.resume) + '.pt'))) # 05 means: fake_ratio=0.5
    # training spa-data
